...
<div class="my-6 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden bg-white dark:bg-gray-800">
	...
	<div class="grid grid-cols-1 lg:grid-cols-2">
		...
		<div class="relative border-b lg:border-b-0 lg:border-r border-gray-200 dark:border-gray-700">
			...
			<pre class="p-4 overflow-x-auto text-sm bg-gray-900 dark:bg-gray-950"><code class="language-{language} font-mono">{@html getHighlightedCode()}</code></pre>
		</div>
			...
			<div class="space-y-3">
				{#each explanations as exp}
					<div class="flex gap-3">
						<div class="flex-shrink-0 w-6 h-6 rounded-full bg-primary-100 dark:bg-primary-900 text-primary-700 dark:text-primary-300 flex items-center justify-center text-xs font-bold">
							{exp.line || '•'}
						</div>
						<div>
							{#if exp.code}
								<code class="text-xs bg-gray-200 dark:bg-gray-700 px-1.5 py-0.5 rounded text-primary-600 dark:text-primary-400">{exp.code}</code>
							{/if}
							<p class="text-sm text-gray-600 dark:text-gray-400 mt-0.5">{exp.text}</p>
						</div>
					</div>
				{/each}
			</div>
